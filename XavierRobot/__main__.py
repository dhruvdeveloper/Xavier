...
import XavierRobot.modules.sql.users_sql as sql
from XavierRobot import (
    BOT_NAME,
    BOT_USERNAME,
    DONATION_LINK,
    HELP_IMG,
...
from XavierRobot.modules import ALL_MODULES
from XavierRobot.modules.helper_funcs.chat_status import is_user_admin
from XavierRobot.modules.helper_funcs.misc import paginate_modules
...
PM_START_TEXT = """
Hey {} [💫](https://te.legra.ph/file/e52cdedb7e05573198f44.mp4),
➤ This is {BOT_NAME} 
➤ A Powerful Group Management bot for protect your group from spammers based on Python

I have alot of features like
↣ Anti-Flood
↣ Approvals
↣ Bans/Mutes
↣ Locks
...
buttons = [
    [
        InlineKeyboardButton(
            text="Add me in Your Group", url=f"https://telegram.dog/{BOT_USERNAME}?startgroup=true"
        ),
    ],
    [
        InlineKeyboardButton(
            text=" Support", url="http://T.me/xaviersupportS"
        ),
        InlineKeyboardButton(
            text="Updates", url="http://t.me/XAVIER_BOTS"
        ),
    ],
    [
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module(f"XavierRobot.modules.{module_name}")
    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
            if args[0].lower() == "help":
                ...
                chat = Xavier_PTB.bot.getChat(match[1])
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton(
                            text="Updates",
                            url="https://telegram.dog/XAVIER_BOTS",
                        ),
                    ]
                ]
            ),
...
def error_handler(update: Update, context: CallbackContext):
    ...
    Xavier_PTB.bot.send_message(chat_id=OWNER_ID, text=message, parse_mode=ParseMode.HTML)
...
def Xavier_callback_data(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    uptime = get_readable_time((time.time() - StartTime))
    if query.data == "Xavier_":
        query.message.edit_text(
            text="""CallBackQueriesData Here""",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(text="Back", callback_data="Xavier_back")]]
            ),
        )
    elif query.data == "Xavier_back":
        first_name = update.effective_user.first_name
        query.message.edit_text(
            PM_START_TEXT.format(
...
def send_settings(context: CallbackContext, chat_id, user_id, user=False):
    if user:
        ...
        chat_name = Xavier_PTB.bot.getChat(chat_id).title
        Xavier_PTB.bot.send_message(
            user_id,
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="Settings",
                            url=f"https://telegram.dog/{Xavier_PTB.bot.username}?start=stngs_{chat.id}",
                        )
                    ]
                ]
            ),
...
def main():
    ...
    data_callback_handler = CallbackQueryHandler(
        Xavier_callback_data, pattern=r"Xavier_", run_async=True
    )
    ...
    Xavier_PTB.add_handler(start_handler)
    Xavier_PTB.add_handler(help_handler)
    Xavier_PTB.add_handler(data_callback_handler)
    Xavier_PTB.add_handler(settings_handler)
    Xavier_PTB.add_handler(help_callback_handler)
    Xavier_PTB.add_handler(settings_callback_handler)
    Xavier_PTB.add_handler(migrate_handler)
    Xavier_PTB.add_handler(donate_handler)

    Xavier_PTB.add_error_handler(error_callback)
    ...
    if WEBHOOK:
        ...
        LOGGER.info(
            f"Xavier started, Using long polling. | BOT: [@{Xavier_PTB.bot.username}]"
        )
