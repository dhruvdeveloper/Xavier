...
def get_user_list(config, key):
    with open(f"{os.getcwd()}/XavierRobot/{config}", "r") as json_file:
...
class Config(object):
    ...
    OWNER_USERNAME = "NOTAAKASH"
    SUPPORT_CHAT = "XavierSupport"  # Your own group for support, do not add the @
    JOIN_LOGGER = (
        -1001739802989
...
class Config(object):
    ...
    OWNER_ID = 1732814103  # If you dont know, run the bot and do /id in your private chat with it, also an integer
    OWNER_USERNAME = "NotAAKASH"
    SUPPORT_CHAT = "XAVIERBOTS_SUPPORT"  # Your own group for support, do not add the @
    JOIN_LOGGER = (
        -1001739802989
    )  # Prints any new group the bot is added to, prints just the name and ID.
    EVENT_LOGS = (
